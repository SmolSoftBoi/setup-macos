...
* **Logical Grouping (Optional):** If you need to group certain apps (for example, all Adobe apps together at the top of the cask list), that’s fine—keep each group sorted alphabetically. Lines beginning with `#` are treated as comments and ignored by the setup script, so you may comment out apps or add headings. Inline comments after an entry are also permitted. Blank lines are ignored.
...
* **Comments:** Lines beginning with `#` are ignored by the setup script. Use them to temporarily disable an app or add a note. Anything after `#` on a line is treated as a comment. Blank lines are also ignored. Keep actual entries one per line and sorted alphabetically.
